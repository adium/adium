...
typedef enum {
	...
	JABBER_STREAM_ATTEMPTING_RESUME
} JabberStreamState;
...
struct _JabberStream
{
	...
	GList *google_relay_requests; /* the HTTP requests to get */
												/* relay info */
	/* stream management stuff */
	gboolean stream_managed;

	char *resume_key;
	guint32 acknowledged_stanzas;
	guint32 handled_stanzas;
	GList *unacknowledged_stanzas;
	char *resume_location;

	guint cancel_resume_timer;
	char *pre_resumption_desc;
};
